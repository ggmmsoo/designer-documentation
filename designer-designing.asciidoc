...
[guilabel]#Outline# gives more control.
...
point, then place the resulting HTML in the hidden [filename]#templates# directory:
[filename]#~/.vaadin/designer/templates# in UNIX systems and
`C:\Users++\++[replaceable]##<username>##\.vaadin\designer\templates` in Windows. It will show
up in the [guilabel]#New Design# wizard as a template (without the [filename]#.html# extension).
...
In Vaadin there are multiple types of resources.
