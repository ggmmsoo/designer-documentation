...
[[designer.designing.using.resources]]
=== Using Resources

Some properties represent a [classname]#Resource# object that should be loaded.
This includes the Icon property, and the Source property for some components.
In Vaadin there are multiple different types of Resources.
...
|ExternalResource
|http[s]:// ftp[s]://
|Browser loads the Resource from given address.

|ThemeResource
|theme://
|Resource is loaded from the application theme folder.

|FileResource
|file://
|Resource is loaded from the server filesystem.

|FontIcon |fonticon://
|Font icon path. Only for the Icon component.
...
Please refer to <<dummy/../../../framework/application/application-resources#application.resources, "Images and Other Resources">> for more information about Resources.
