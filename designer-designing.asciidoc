...
required. Replacing the layout that contains those components is the best
option. To replace a layout, click on the layout to be replaced in the
[guilabel]#Outline# view, choose [menuchoice]#Replace with# in the context menu 
and select a layout to replace the current one with from the list of layouts
that appears.
...
[classname]#ComponentContainer# with a [classname]#SingleComponentContainer# 
the leftover compoents will be dropped. Undo and redo work as usual, so you
